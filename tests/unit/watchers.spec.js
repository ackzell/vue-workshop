...
describe("Watchers tests", () => {
  xit("should have a `myFavoriteFlavor` watcher implemented", async () => {
    expect(Watchers.watch.myFavoriteFlavor).toBeDefined();
  });

  xit("should append a new entry on the logs every time the value of `myFavoriteFlavor` changes", async () => {
  ...
  xit("should display the old value as well as the new value on each entry", async () => {
    const wrapper = mount(Watchers);
    await wrapper.find("#fav-b-Chocolate").trigger("click");
    await wrapper.find("#fav-b-Vanilla").trigger("click");
    const logs = wrapper.findAll(".log-entry");
